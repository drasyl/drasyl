...
package org.drasyl.core.server.monitoring.server;

import org.drasyl.core.server.RelayServer;
import org.drasyl.core.server.monitoring.Aggregator;
...
//                .handler(new LoggingHandler(LogLevel.INFO))
        ...
        LOG.info("WebSocket for monitoring was started on port: {}", port);
    }
    ...
    private String getBaseDir() {
        File dir;
        if (debuggingMode) {
            dir = new File("server/relay-monitoring-page/dist");
        } else {
            dir = new File("public/");
            if (!dir.exists()) {
                String path = RelayServer.class.getProtectionDomain().getCodeSource().getLocation().getPath();
