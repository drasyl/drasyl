...
public final class ChordFindPredecessorHelper {
    ...
    private static Future<IdentityPublicKey> recursive(final ChannelHandlerContext ctx,
        ...
        if (pre_n.equals(ctx.channel().localAddress())) {
            return chainFuture(closestPrecedingFinger(ctx, findid, fingerTable), ctx.executor(), n -> {
                if (pre_n.equals(n)) {
        ...
        // else current node is remote node, sent request to it for its closest
            ...
            return chainFuture(closestRequest(ctx, pre_n, findid), ctx.executor(), result -> {
                // if fail to get response, set n to most recently
                if (result == null) {
                    return chainFuture(yourSuccessorRequest(ctx, most_recently_alive), ctx.executor(), n_successor -> {
                        if (n_successor == null) {
                ...
                // else n's closest is other node "result"
                    ...
                    return chainFuture(yourSuccessorRequest(ctx, result), ctx.executor(), n_successor -> {
