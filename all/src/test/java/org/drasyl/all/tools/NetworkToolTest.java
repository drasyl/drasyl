...
public class NetworkToolTest {
    ...
    public void availablePortTest() {
        try (ServerSocket socket = new ServerSocket(5555)) {
            assertFalse(NetworkTool.available(5555));
        } catch (IOException e) {
        }

        assertTrue(NetworkTool.available(4444));
    }
    ...
    public void aliveTest() {
        try (ServerSocket socket = new ServerSocket(2222)) {
            assertTrue(NetworkTool.alive("127.0.0.1", 2222));
        } catch (IOException e) {
        }

        assertFalse(NetworkTool.alive("127.0.0.1", 3333));
    }
}
