...
public abstract class ClientInitializer extends DefaultSessionInitializer {
    ...
    protected ClientInitializer(int flushBufferSize, Duration readIdleTimeout, int pingPongRetries,
                                int maxContentLength, IPAddress target,
    ...
    protected void beforeMarshalStage(ChannelPipeline pipeline) {
        try {
            ...
            if (pipeline.get("sslHandler") != null) {
                protocol = "wss://";
            }

            WebSocketClientHandler webSocketClientHandler =
                    new WebSocketClientHandler(WebSocketClientHandshakerFactory.newHandshaker(new URI(protocol + target.toString() + "/"), // NOSONAR
            ...
            LOG.error("Can't set WebSocket URI: ", e);
            channelReadyFuture.completeExceptionally(e);
            throw new IllegalArgumentException("Can't set WebSocket URI: " + target.toString());
        }
    }
